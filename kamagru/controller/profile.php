...
if (isset($_SESSION['co']) && $_SESSION['co']) {
	if (isset($_GET['action']) && !empty($_GET['action']) && getToken('action')) {
		$a = $_GET["action"];
		if ($a == "pwd" && verif_password($_POST['pwd1']))
			request($_POST['pwd'], $_POST['pwd1'], $_POST['pwd2']);
		if ($a == "email" && ($_POST['email'] == $_POST['email1']) && verif_email($_POST['email1']))
			if(changeme(conn_db(), "UPDATE user SET email = :login WHERE login like :haha", $_POST['email1'], $_SESSION['login']))
				$_SESSION['email'] = $_POST['email1'];
		if ($a == "login" && ($_POST['login'] == $_POST['login1']) && strlen(trim($_POST['login'])) > 2)
			if (changeme(conn_db(), "UPDATE user SET login = :login WHERE login like :haha", $_POST['login1'], trim($_SESSION['login'])))
				$_SESSION['login'] = trim($_POST['login1']);
		if ($a == "accept")
			changeme(conn_db(),"UPDATE user SET accept = :login WHERE login like :haha", is('accept'), $_SESSION['login']);
	}
